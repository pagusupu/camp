{
  ...
  options.cute.system.misc.synapse = lib.mkEnableOption "";
  config = let
    domain = "${config.cute.system.web.domain}";
    matrix_domain = "matrix.${config.cute.system.web.domain}";
    port = 8008;
  in
    lib.mkIf config.cute.system.misc.synapse {
      services = {
        matrix-synapse = {
          enable = true;
          withJemalloc = true;
          ...
          dataDir = "/storage/services/synapse";
        };
        postgresql = {
          ensureUsers = [
            {
